...
- 🔐 **Fine-Grained RBAC & Dynamic Permission Gating**: Declarative `<PermissionGate />` components, custom authorization hooks (`useAuthorization`), and protected route guards.
- 🔀 **N-Level Extensible Workflow Engine**: Data-driven multi-tier approval state machine, visual stepper, audit timeline, and signature revocation.
- 📊 **Executive ERP Dashboard**: Dynamic drag-and-drop widget reordering, column span toggling, auto-sync polling countdown, and `localStorage` layout persistence.
...
- 📊 **Real-Time Telemetry & Aggregation**: Unified CQRS metrics query (`GetDashboardMetricsQuery`), EF Core SQL projections, and CLR Garbage Collector / ThreadPool diagnostics.
...
- 🧪 **Automated Testing & CI/CD**: 55 xUnit tests (Moq, FluentAssertions, EF Core InMemory) and automated GitHub Actions CI/CD workflows.
...
│   │   ├── features/                     # Feature modules (Auth, Products, Payments/Idempotency, Workflows)
